...
async function applyMigration() {
  const migrationPath = path.join(__dirname, "..", "supabase", "migrations", "20260610000000_vx_roles.sql");
  ...
  try {
    ...
    console.log("Conectado. Aplicando migration vx_roles...");
    await client.query(sql);
    console.log("Migration vx_roles aplicada com sucesso!");
  } catch (err) {
